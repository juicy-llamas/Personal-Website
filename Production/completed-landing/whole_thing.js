...
const fn = () => {
	...
	
	{
		...
		const fbt0 = gl.createTexture();
		gl.bindTexture( gl.TEXTURE_2D, fbt0 );
		gl.texImage2D( gl.TEXTURE_2D, 0, gl.RGBA, canvas.width, canvas.height, 0, gl.RGBA, gl.UNSIGNED_BYTE, null, 0 );
		gl.bindFramebuffer( framebuffers[ 0 ] );
		gl.framebufferTexture2D( gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, fbt0, 0 );
		
		gl.activeTexture( gl.TEXTURE1 );
		const fbt1 = gl.createTexture();
		
		gl.bindTexture( gl.TEXTURE_2D, fbt1 );
		gl.texImage2D( gl.TEXTURE_2D, 0, gl.RGBA, canvas.width, canvas.height, 0, gl.RGBA, gl.UNSIGNED_BYTE, null, 0 );
		
		gl.bindFramebuffer( framebuffers[ 1 ] );
		gl.framebufferTexture2D( gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, fbt1, 0 );
	}
